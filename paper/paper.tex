...
%While ideally we would include evolution in the SMHM relation, but evolution introduces significant complexity into the CAM modelling when including assembly bias.  In principle, the stellar mass growth history of each (sub-)halo should be correlated with formation history up to that point, such that the stellar mass function at each redshift is reproduced.  In this way, a truly consistent model for stellar mass growth could be implemented; however, such a model is beyond the scope of this paper.  Instead, we simply examine the degree and phenomena of such a model on the galaxy clustering.  Using a non-evolving SMHM relation will is a logical place to begin. 
